...
public class ClientCnxnSocketNIO extends ClientCnxnSocket {
        ...
        if (sockKey.isWritable()) { //写就绪
            synchronized(outgoingQueue) {
                ...
                if (p != null) {
                    ...
                    if (!p.bb.hasRemaining()) { //当前packet没有剩余数据需要发送，也就是发送完成后的处理逻辑
                        sentCount++;
                        outgoingQueue.removeFirstOccurrence(p); //从outgoingQueue从移除已经处理的packet
                        if (p.requestHeader != null
                                && p.requestHeader.getType() != OpCode.ping
                                && p.requestHeader.getType() != OpCode.auth) {
    ...
    throws IOException {
        ...
        boolean immediateConnect = sock.connect(addr); //有没有立刻连接成功
        if (immediateConnect) {
    ...
    void connect(InetSocketAddress addr) throws IOException {
        SocketChannel sock = createSock();//真正的建立socket
        try {
