...
public class ClientCnxn {
    ...
    private void finishPacket(Packet p) {
        if (p.watchRegistration != null) {
            //客户端先发送消息给服务端
            //服务端返回消息后，在这里才会注册/nodename : watcher的对应关系
    ...
    class SendThread extends ZooKeeperThread {
        ...
        /*
            对于TCP连接，服务器上监听端口只是第一步，收到新的连接请求并accept之后，客户端和服务器间会建立一条持久的连接，在服务器的代码上看，就是创建了一个新的socket。往这个socket上写内容，客户端就可以收到了。
            也就是建立好连接后，就没有服务端与客户端之分了，都可以相互发送消息，写进socket,  对方就可以收到，当然不要求对方回复。
            这就是双工模型
         */
        void readResponse(ByteBuffer incomingBuffer) throws IOException {
            ...
            //-1 代表服务端发来的通知，客户端会把有没有注册watcher的信息高速服务端
            //服务端收到是true的话，节点发生相应的变化时就会发送一个通知
            //这些事件会存在EventThread.waitingEvents队列中
        ...
        void primeConnection() throws IOException {
            ...
            //ConnectRequest 连接请求 ，也会加入outgoingqueue
        ...
        private void startConnect(InetSocketAddress addr) throws IOException {
            ...
            state = States.CONNECTING; //改为连接中的状态
            ...
            clientCnxnSocket.connect(addr); //建立连接
        }
        ...
        public void run() {
            ...
            while (state.isAlive()) {
                try {
                    if (!clientCnxnSocket.isConnected()) { //注释 如果socket还没有连接
                        if(!isFirstConnect){
                            try {
                    ...
                    //to 为负数表示超时， 抛异常，表示需要重试，while循环中重试
                    if (to <= 0) {
                        String warnInfo;
