...
public class ZooKeeper {
    ...
    private static class ZKWatchManager implements ClientWatchManager {
                                        ...
                                        String clientPath)
        {
            ...
            case NodeCreated:
                synchronized (dataWatches) {
                    //原生的客户端每个监听器只能响应一次事件，就是因为这里获取到watcher后，就从dataWatches把这个watcher删除了
    ...
    public byte[] getData(final String path, Watcher watcher, Stat stat)
        throws KeeperException, InterruptedException
     {
        ...
        //只是把有没有注册watch , 也就是true/false告诉服务端
        request.setWatch(watcher != null);
    ...
    private static ClientCnxnSocket getClientCnxnSocket() throws IOException {
        //先查询有没有配置socket的类名,是一个可以扩展的点，我们可以指定我们自己写的socket类
        String clientCnxnSocketName = System
                .getProperty(ZOOKEEPER_CLIENT_CNXN_SOCKET);
        //如果没有配置，就默认使用ClientCnxnSocketNIO这个类
        if (clientCnxnSocketName == null) {
            clientCnxnSocketName = ClientCnxnSocketNIO.class.getName();
        }
        try {
            //使用socketnio进行客户端与服务端的通信
